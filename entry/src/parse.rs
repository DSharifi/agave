...
fn read_entry(bytes: &Bytes, offset: &mut usize) -> Result<Entry, EntryParseError> {
    ...
    // allocation check even though Entry now stores the smaller view metadata.
...
fn check_preallocation<T>(count: usize) -> Result<(), EntryParseError> {
    ...
    let needed = count
        ...
        .ok_or(EntryParseError::PreallocationLimit {
            count,
            element_size,
...
mod tests {
    ...
    fn test_trailing_bytes_are_accepted_and_reported() {
        ...
        assert_eq!(entries_from_bytes(&serialized).unwrap(), entries);
    }
