...
pub struct Record {
    pub mixin: Hash,
    pub transactions: Vec<EntryTransaction>,
    pub bank_id: BankId,
}

impl Record {
    pub fn new(mixin: Hash, transactions: Vec<EntryTransaction>, bank_id: BankId) -> Self {
        Self {
            mixin,
            transactions,
            bank_id,
...
mod tests {
    use {
        ...
        solana_entry::entry::{
            self, EntrySlice, EntryTransaction, transaction_view_from_versioned_transaction,
        },
    ...
    fn entry_transaction(transaction: Transaction) -> EntryTransaction {
        transaction_view_from_versioned_transaction(&VersionedTransaction::from(transaction))
    }
    ...
    fn test_poh_recorder_record_to_early() {
        ...
        assert_matches!(
            poh_recorder.record(bank1.slot(), h1, vec![entry_transaction(tx)]),
    ...
    fn test_poh_recorder_record_bad_slot() {
        ...
        assert_matches!(
            poh_recorder.record(bad_slot, h1, vec![entry_transaction(tx)]),
    ...
    fn test_poh_recorder_record_at_min_passes() {
        ...
        assert!(
            poh_recorder
                .record(bank1.slot(), h1, vec![entry_transaction(tx)])
                .is_ok()
        );
        assert_eq!(poh_recorder.tick_cache.len(), 0);
    ...
    fn test_poh_recorder_record_at_max_fails() {
        ...
        assert!(
            poh_recorder
                .record(bank.slot(), h1, vec![entry_transaction(tx)])
                .is_err()
        );
        for _ in 0..num_ticks_to_max {
