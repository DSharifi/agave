...
pub mod tests {
    ...
    fn create_test_transactions(
        ...
        vec![
            system_transaction::transfer(mint_keypair, &pubkey, 1, *genesis_hash).into(),
            system_transaction::transfer(&keypair2, &pubkey2, 1, *genesis_hash).into(),
            system_transaction::transfer(&keypair3, &pubkey3, 1, *genesis_hash).into(),
        ]
    }
    ...
    fn test_validate_entry_transactions_ok() {
        ...
        let txs = vec![
            system_transaction::transfer(&payer, &Pubkey::new_unique(), 1, hash).into(),
            system_transaction::transfer(&payer, &Pubkey::new_unique(), 1, hash).into(),
        ];
    ...
    fn test_validate_entry_transactions_account_loaded_twice() {
        ...
        let txs = vec![Transaction::new(&[&payer], message, Hash::new_unique()).into()];
        assert_eq!(
